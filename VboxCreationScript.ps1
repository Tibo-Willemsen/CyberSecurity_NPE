...
if ($ExistingVMs -match "`"$VM_TARGET_NAME`"") {
    ...
    VBoxManage modifyvm "$VM_TARGET_NAME" --nic2 hostonly --hostonlyadapter1 "VirtualBox Host-Only Ethernet Adapter"
}
# Copy the setup script from your Windows host to the VM's /tmp folder
VBoxManage guestcontrol "$VM_TARGET_NAME" copyto --username $USER --password $PASS "C:\Users\wille\CyberSecVirt\CyberSecurity_NPE\setup\setup_redis.sh" "   "
